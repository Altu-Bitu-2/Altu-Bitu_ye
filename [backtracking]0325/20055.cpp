...
//벨트의 로봇을 옮기는 대신 올리는 위치와 내리는 위치의 인덱스 값의 인덱스 감소
int minusPosition(int n, int pos) { //인덱스 감소
    if (--pos >= 0) {
        return pos;     //하나 감소한 인덱스 값 리턴
    }
    return n * 2 - 1;   //인덱스가 0보다 작아지면 마지막 인덱스 값 리턴
}

void second(int n, int start, int end) {
    int cur = end;
    while (cur != start) { //end - 1 부터 start 까지 검사
        cur = minusPosition(n, cur);    //현재 위치
        int next = (cur + 1) % (n * 2); //다음 위치. 현재 위치 + 1
                                        //다음 위치의 인덱스가 2N이상일 수 있으므로 나머지 연산
        //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구도 1이상인 경우 -> 로봇 옮김
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) {
            belt[cur].is_on = false;    //현재 위치에 로봇 여부 없음으로 변경
            belt[next].power--;         //로봇이 이동한 칸의 내구도 1 감소
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) {    //내구도가 0인 칸이 발생하면
                zero_power++;   //내구도가 0인 벨트 칸의 개수 1 증가
            }
        }
    }
}
...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        belt[start].is_on = true;   //로봇 여부 있음으로 변경
        belt[start].power--;        //로봇을 올리는 위치인 칸의 내구도 1 감소
        if (belt[start].power == 0) {   //로봇을 올리는 위치인 칸의 내구도가 0이면
            zero_power++;   //내구도가 0인 벨트 칸의 개수 1 증가
        }
    }
}
...
int main() {
    ...
    int n, k;   //벨트의 길이, 내구도가 0인 칸의 개수가 k개 이상일 시 종료

    //입력
    cin >> n >> k;
    belt.assign(n * 2, {0, false}); //벨트 칸의 개수만큼 (내구도 0, 로봇 없음)으로 초기화
    for (int i = 0; i < n * 2; i++) {
    ...
    while (true) {
        //1번 과정
        start = minusPosition(n, start);    //올리는 위치 인덱스 값 하나 감소
        end = minusPosition(n, end);        //내리는 위치 인덱스 값 하나 감소
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false;
        }
        second(n, start, end);  //2번 과정
        third(start);   //3번 과정
        step++; //한 단계 종료. 단계 수 증가
        if (zero_power >= k) {  //4번 과정.
            break;  //내구도가 0인 칸의 개수가 K개 이상이라면 과정 종료.
        }
    }
