...
using namespace std;
const int MOD = 10007;  //방법의 수를 10,007로 나눔
...
int fillTile(int n) {
    ...
    if (n == 1) {   //인덱스 에러 안나도록 n이 1일 경우 바로 리턴
        return 1;   //너비 1은 2*1 타일로 채우는 경우(1가지)밖에 없음
    }

    //너비 1, 2인 타일 채우는 경우의 수 미리 초기화
    dp[1] = 1;
    dp[2] = 3;  //너비 2는 1*2 2개, 2*2 1개, 2*1 2개의 3가지 경우
    for (int i = 3; i <= n; i++) {
        //(너비 1인 타일 채우는 경우의 수 = 1) * dp[n - 1]
        // + (너비 2인 타일 채우는 경우의 수 = 2) * dp[n - 2]
        //n - 1번째까지 채우면 나머지는 너비가 1인 타일로만 채울 수 있고
        //n - 2번째까지 채우면 나머지는 너비가 2인 타일로만 채울 수 있음
        dp[i] = dp[i - 1] + 2 * dp[i - 2];
        dp[i] %= MOD; //구하는 과정에서 int 범위 초과할 수 있으므로 마지막에 한 번이 아니라 중간 중간 모듈러 연산을 해줘야 함
    }
    return dp[n];   //방법의 수 % MOD 리턴
}
