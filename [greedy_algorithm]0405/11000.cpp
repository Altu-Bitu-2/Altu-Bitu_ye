...
int arrayRoom(int n, vector<ci> lec) {
    ...
    for (int i = 0; i < n; i++) {
        if (lec[i].first >= pq.top()) { //종료시간이 가장 빠른 수업보다 시작시간이 같거나 느리다면 -> 같은 강의실
            pq.pop();                   //같은 강의실이므로 앞 수업의 종료 시간 제거
        }
        pq.push(lec[i].second); //기존 강의실 정보 갱신(같은 강의실 사용한 경우)
                                //or 새로운 강의실 정보 저장
    }
    return pq.size();   //사용한 강의실의 개수 리턴
}
...
int main() {
    ...
    //입력
    cin >> n;
    vector<ci> lec(n, ci(0, 0));    //수업 시작 시간(si), 수업 종료 시간(ti)
    for (int i = 0; i < n; i++) {
